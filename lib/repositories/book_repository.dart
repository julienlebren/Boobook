...
final bookRepositoryProvider = Provider<BookRepository>((ref) {
  ...
  final service = ref.watch(firestoreServiceProvider);
  return BookRepository(service, userRepository);
});
...
class BookRepository {
  BookRepository(
    ...
    _bookRef = _userRepository.ref<Book>(
      FirestorePath.books,
      fromFirestore: (snapshot, _) {
        final bookFromJson = Book.fromJson(snapshot.data()!);
        return bookFromJson.copyWith(id: snapshot.id);
    ...
    _bookLibraryRef = FirebaseFirestore.instance
        .collection(FirestorePath.books)
        .withConverter<Book>(
      fromFirestore: (snapshot, _) {
        final bookFromJson = Book.fromJson(snapshot.data()!);
      ...
      toFirestore: (book, _) {
        return book.copyWith(userId: _userRepository.userId).toJson()
          ..remove("id")
          ..remove("isFromISBNdb")
          ..remove("isFromUnknownISBN")
          ..remove("isNewBook");
    ...
    _bookStorageRef = _userRepository.storageRef(StoragePath.books);
  }

  final FirestoreService _service;
  final UserRepository _userRepository;

  late CollectionReference<Book> _bookRef;
  late CollectionReference<Book> _bookLibraryRef;
  late Reference _bookStorageRef;
